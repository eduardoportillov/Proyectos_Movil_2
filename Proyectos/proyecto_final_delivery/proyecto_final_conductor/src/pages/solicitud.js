...
export default class solicitud extends Component {
    ...
    getMapa() {
        ...
        const { latitudOrigen, longitudOrigen, latitudDestino, longitudDestino } = this.state.data;
        return <MapView
            provider={'google'}
            region={{
    ...
    getFooter() {
        if (!this.state.data) return null;
        const { latitudOrigen, longitudOrigen, latitudDestino, longitudDestino, precio, estado } = this.state.data;

        return <SView width height={200} bg={"#ffffff"} style={{
            ...
            <SView height={16} />
            <Text style={{ fontSize: 20, fontWeight: "bold" }}>Bs. {precio}</Text>
            <SView height={16} />

            <SView bg={"#ff6666"} p={8} br={4} onPress={() => {
                Model.entrega.aceptar({ id: this.state.id, token: this.state.usr.access_token }).then((e) => {
                    this.props.navigation.replace("entrega", { id: this.state.id });
                })
            }}>
                <Text style={{ fontSize: 18, color: "#fff", fontWeight: "bold" }}>Aceptar el viaje</Text>
            </SView>
        </SView>
    }
    render() {
